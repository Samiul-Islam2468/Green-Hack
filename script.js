...
function showSection(sectionId) {
    const sections = [
        ...
        'ngoResultsTable', 
        'ngoContactScreen', 
        'ambulanceView', // NEW SECTION ADDED
        'adminDashboard'
    ];
...
function loadAmbulances() {
    ...
    if (userLat && userLng) {
        results.forEach(amb => {
            amb.distance = calculateDistance(userLat, userLng, amb.lat, amb.lng);
        });
        // Sort closest first
    ...
    results.forEach(amb => {
        ...
        // Random availability: Ambulances are highly active, simulating 80% available
        ...
        tbody.innerHTML += `
            <tr>
                <td>
                    ...
                    <span style="color:var(--text-main); font-size:14px; font-weight:600;">📞 ${amb.phone}</span>
                    <div style="margin-top: 5px;">${availHtml} ${distHtml}</div>
                </td>
                <td style="color: #4b5563;">${amb.address}</td>
                <td>
                    <button class="btn" style="margin-top:0; padding: 10px 18px; border-radius: 8px; width: 100%; ${btnStyle}" ${btnDisabledAttr} onclick="alert('Calling Dispatch: ${amb.name} at ${amb.phone}...')">
                        🚨 Call Now
                    </button>
                </td>
            </tr>
...
// --- NEW: DYNAMIC PAYMENT CALCULATION ---
function updatePaymentDetails() {
    ...
    const scheduledDateObj = new Date(dateVal);
    scheduledDateObj.setHours(0, 0, 0, 0); 
    
    const deadlineObj = new Date(scheduledDateObj.getTime() - (24 * 60 * 60 * 1000));
    
    const deadlineString = deadlineObj.toLocaleDateString('en-IN') + " at 11:59 PM";
