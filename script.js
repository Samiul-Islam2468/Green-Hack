...
// --- NEW GLOBALS FOR PAYMENT LOGIC ---
let currentSearchType = 'blood'; // Defaults to blood
let currentUnits = 1;
let calculatedAdvanceAmount = 0; // Stores the final ₹ amount to pass to Razorpay

// BASE PRICES
const BASE_PRICE_BLOOD_PER_UNIT = 1500; // ₹1,500 per unit of blood
const BASE_PRICE_ORGAN = 50000;         // ₹50,000 flat booking rate for organs
const BASE_PRICE_AMBULANCE = 500;       // ₹500 fixed emergency booking fee

// --- FULL WEST BENGAL HOSPITAL DATABASE ---
const hospitalData = [
...
// --- NEW AMBULANCE DATABASE ---
const ambulanceData = [
    { id: "WB-01-1234", name: "Life Care Emergency", type: "ALS (Advanced Life Support)", phone: "102", lat: 22.5726, lng: 88.3639, address: "Central Kolkata Fleet" },
    { id: "WB-02-5678", name: "City Rescue Medical", type: "BLS (Basic Life Support)", phone: "09876543210", lat: 22.6288, lng: 88.4443, address: "Newtown Action Area" },
    { id: "WB-04-9999", name: "Hope ICU Transport", type: "ICU Ambulance", phone: "09123456789", lat: 22.5831, lng: 88.4026, address: "Salt Lake Sector 1 Depot" },
    { id: "WB-05-1111", name: "Quick Response Unit", type: "BLS (Basic Life Support)", phone: "09000011111", lat: 22.4939, lng: 88.3181, address: "Behala South Point" },
    { id: "WB-06-2222", name: "Highway Trauma Rescue", type: "ALS (Advanced Life Support)", phone: "09999922222", lat: 26.7271, lng: 88.4316, address: "Siliguri Main Base" },
    { id: "WB-08-3333", name: "Sanjivani Swift", type: "Patient Transport", phone: "09831122334", lat: 22.5132, lng: 88.4031, address: "Kasba Area" }
];
...
const dictionary = {
    'en': {
        ...
        'card3Title': 'Emergency Ambulance',
        'card3Desc': 'Find and contact exact nearby ambulances instantly for emergencies.',
        'ambTitle': 'Nearby Ambulances',
        'colAmbName': 'Ambulance Details',
    ...
    'bn': {
        ...
        'card3Title': 'জরুরী অ্যাম্বুলেন্স',
        'card3Desc': 'জরুরী অবস্থার জন্য অবিলম্বে নিকটস্থ অ্যাম্বুলেন্স খুঁজুন এবং বুক করুন।',
        'ambTitle': 'নিকটবর্তী অ্যাম্বুলেন্স',
        'colAmbName': 'অ্যাম্বুলেন্সের বিবরণ',
...
function validateAndSearch() {
    ...
    currentUnits = isBlood && units !== "" ? parseInt(units) : 1;
...
function validateAndSearchNgo() {
    ...
    let results = ngoData.filter(function(ngo) { return ngo.district === selectedDistrict; });
...
function loadAmbulances() {
    showToast("Locating nearby emergency fleets...", "success");
    
    let results = [...ambulanceData];
    if (userLat && userLng) {
        results.forEach(amb => { amb.distance = calculateDistance(userLat, userLng, amb.lat, amb.lng); });
        results.sort((a, b) => a.distance - b.distance);
    }

    const tbody = document.getElementById('dynamicAmbulanceBody');
    if (!tbody) return;
    tbody.innerHTML = "";
    ...
    results.forEach(amb => {
        let distHtml = '';
        if (amb.distance) {
            distHtml = `<br><span class="dist-badge" style="background-color:#fee2e2; color:#991b1b;">📍 ${amb.distance} km away</span>`;
        }
        ...
        if (isAvailable) {
            availHtml = `<span class="avail-badge success">✅ Available Now</span>`;
            btnStyle = 'background: var(--error-red);';
        } else {
            availHtml = `<span class="avail-badge danger">❌ On Duty (Busy)</span>`;
            btnDisabledAttr = 'disabled';
            btnStyle = 'background: var(--text-muted); cursor: not-allowed; opacity: 0.6;';
        }

        tbody.innerHTML += `
            <tr>
                <td>
                    <strong style="color: var(--error-red); font-size: 16px;">${amb.name}</strong><br>
                    <span style="font-size: 13px; color: var(--text-muted);">${amb.type} • Reg: ${amb.id}</span><br>
                    <div style="margin-top: 5px;">${availHtml} ${distHtml}</div>
                </td>
                <td style="color: #4b5563;">${amb.address}</td>
                <td>
                    <button class="btn" style="margin-top:0; padding: 10px 18px; border-radius: 8px; width: 100%; ${btnStyle}" ${btnDisabledAttr} onclick="bookAmbulance('${amb.name}')">
                        🚨 Book Now
                    </button>
                </td>
            </tr>
...
// Bypass Date Picker for Immediate Dispatch
function bookAmbulance(ambName) {
    currentSearchType = 'ambulance';
    selectedHospital = ambName; 
    const display = document.getElementById('hospitalNameDisplay');
    if (display) display.innerText = ambName;
...
// --- DYNAMIC RENDERING (Hospitals & NGOs) ---
function renderTableAndMap(filteredData) {
    ...
    filteredData.forEach(function(hosp) {
        let distHtml = '';
        if (hosp.distance) {
            distHtml = `<span class="dist-badge">📍 ${hosp.distance} km away</span>`;
        }
        
        const isAvailable = Math.random() > 0.4;
        let availHtml = '';
        let btnDisabledAttr = '';
        let btnStyleAttr = 'style="margin-top:0; padding: 8px 16px; border-radius: 8px;"';

        if (isAvailable) {
            availHtml = `<span class="avail-badge success">✅ In Stock</span>`;
        } else {
            availHtml = `<span class="avail-badge danger">❌ Out of Stock</span>`;
            btnDisabledAttr = 'disabled';
            btnStyleAttr = 'style="margin-top:0; padding: 8px 16px; border-radius: 8px; opacity: 0.5; cursor: not-allowed;"';
        }
        ...
        tbody.innerHTML += `
            <tr>
                <td>
                    ...
                    <span style="color:var(--text-muted); font-size:13px;">${phoneTxt}</span><br>
                ...
                <td>
                    <button class="btn" ${btnStyleAttr} ${btnDisabledAttr} onclick="bookFacility('${hosp.name}')" data-translate="btnBook">
                        ${btnText}
                    </button>
                </td>
            </tr>
    ...
    if (map && typeof google !== 'undefined') {
        markersArray.forEach(function(marker) { marker.setMap(null); });
        markersArray = [];
        map.setCenter({ lat: filteredData[0].lat, lng: filteredData[0].lng });
        map.setZoom(12);
        filteredData.forEach(function(hosp) {
            let newMarker = new google.maps.Marker({ position: { lat: hosp.lat, lng: hosp.lng }, map: map, title: hosp.name });
...
function bookFacility(hospitalName) { 
    ...
    // Ensure TODAY can be selected
    const today = new Date();
    const minDateString = today.toISOString().split('T')[0];
    document.getElementById('scheduledDate').min = minDateString;

    showSection('paymentScreen'); 
}

// --- DYNAMIC PAYMENT & DEADLINE ENGINE ---
function updatePaymentDetails() {
    const btn = document.getElementById('payBtn');
    const breakdown = document.getElementById('paymentBreakdown');
    const warningBox = document.getElementById('deadlineWarningBox');
    const remDueSec = document.getElementById('remainingDueSection');
    ...
    // 1. Check Booking Type
    if (currentSearchType === 'ambulance') {
        totalCost = BASE_PRICE_AMBULANCE;
        advancePercent = 100;
        isToday = true;
    } else {
        ...
        // 2. Check if Date Selected is Today
        const selectedDateObj = new Date(dateVal);
        const todayObj = new Date();
        
        // Accurate Local timezone date checking
        const yyyy = todayObj.getFullYear();
        const mm = String(todayObj.getMonth() + 1).padStart(2, '0');
        const dd = String(todayObj.getDate()).padStart(2, '0');
        const localTodayStr = `${yyyy}-${mm}-${dd}`;
        
        if (dateVal === localTodayStr) {
            isToday = true;
        }

        // 3. Set Base Pricing Rules
        if (currentSearchType === 'blood') {
            totalCost = BASE_PRICE_BLOOD_PER_UNIT * currentUnits;
            advancePercent = isToday ? 100 : 25; // 100% full payment if today
        } else if (currentSearchType === 'organ') {
            totalCost = BASE_PRICE_ORGAN;
            advancePercent = isToday ? 100 : 65; // 100% full payment if today
        }

        // 4. Calculate Deadline
        if (!isToday) {
            selectedDateObj.setHours(0, 0, 0, 0); 
            const deadlineObj = new Date(selectedDateObj.getTime() - (24 * 60 * 60 * 1000));
            deadlineString = deadlineObj.toLocaleDateString('en-IN') + " at 11:59 PM";
        }
    }

    const duePercent = 100 - advancePercent;
    calculatedAdvanceAmount = (totalCost * advancePercent) / 100;
    const dueAmount = totalCost - calculatedAdvanceAmount;

    // 5. Update UI Text
    document.getElementById('totalCostDisplay').innerText = totalCost.toFixed(2);
    document.getElementById('advancePercentDisplay').innerText = advancePercent;
    document.getElementById('advanceAmountDisplay').innerText = calculatedAdvanceAmount.toFixed(2);
    document.getElementById('duePercentDisplay').innerText = duePercent;
    document.getElementById('dueAmountDisplay').innerText = dueAmount.toFixed(2);

    // Dynamic UI Toggles based on 100% payment
    if (advancePercent === 100) {
        warningBox.style.display = 'none';
        remDueSec.style.display = 'none';
    } else {
        warningBox.style.display = 'block';
        remDueSec.style.display = 'block';
        document.getElementById('deadlineDisplay').innerText = deadlineString;
    }
...
function processPayment() {
    payWithRazorpay(calculatedAdvanceAmount);
}

// --- UPDATED RAZORPAY INTEGRATION ---
async function payWithRazorpay(amountToPay) {
    ...
    const options = {
        ...
        "description": "Advance Token for " + selectedHospital,
        "theme": { "color": "#059669" },
        "prefill": {
            "name": "Samiul Islam",
            "contact": "7601868550"
    ...
    try {
        if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Checkout) {
            ...
            setTimeout(function() { handleSuccessResponse(amountToPay); }, 2000);
        } else {
...
// Handles custom alert messages based on Booking Type
function handleSuccessResponse(amountPaid) {
    const receiptId = Math.floor(Math.random() * 100000);
    let msg = "";
    
    if (currentSearchType === 'ambulance') {
        msg = `🚑 Ambulance Dispatch Confirmed!\n\nYour Dispatch Code is: AMB-${receiptId}\n\nYou will automatically receive a call shortly from the driver for exact tracking.\nAmount Paid: ₹${amountPaid}`;
    } else {
        msg = `✅ Booking Confirmed for ${selectedHospital}!\n\nAn amount of ₹${amountPaid} has been paid.\nYour official receipt ID is: ${receiptId}`;
    }
...
function initMap() { 
    ...
    if (mapElement) {
        map = new google.maps.Map(mapElement, { zoom: 11, center: initialLocation }); 
    }
}
...
async function getLocation() {
    ...
    if (locText) {
        if (currentLang === 'en') locText.innerHTML = "⏳ Fetching your location...";
        else locText.innerHTML = "⏳ অবস্থান আনা হচ্ছে...";
    ...
    try {
        if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Geolocation) {
            ...
            if (permissionStatus.location !== 'granted') throw new Error("User denied native permission");
            
            const position = await window.Capacitor.Plugins.Geolocation.getCurrentPosition({ enableHighAccuracy: true, timeout: 10000 });
        ...
        if (locText) {
            if (currentLang === 'en') locText.innerHTML = `✅ Location captured: Lat ${userLat.toFixed(2)}, Lng ${userLng.toFixed(2)}`;
            else locText.innerHTML = `✅ অবস্থান ক্যাপচার করা হয়েছে: অক্ষাংশ ${userLat.toFixed(2)}, দ্রাঘিমাংশ ${userLng.toFixed(2)}`;
